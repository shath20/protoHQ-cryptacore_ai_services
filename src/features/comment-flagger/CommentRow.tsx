...
export function CommentRow({ comment, onClick, onAction, selected = false, onToggleSelect }: CommentRowProps) {
  ...
  return (
    <div
      className={`group relative p-4 hover:shadow-lg transition-all duration-300 cursor-pointer border-l-4 ${isHighRisk ? 'border-red-500 bg-red-50' :
        comment.aiScore > 70 ? 'border-orange-500 bg-orange-50' :
          comment.aiScore > 40 ? 'border-yellow-500 bg-yellow-50' :
            'border-green-500 bg-green-50'
        } hover:shadow-xl hover:-translate-y-1 transition-all duration-300 transform border-l-4 animate-in fade-in slide-in-from-bottom-2`}
      onClick={onClick}
    >
      {/* Risk Indicator Bar */}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              {comment.indicators.slice(0, 2).map((indicator, index) => (
                <span
                  key={index}
                  className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${indicator.includes('Trademark') || indicator.includes('Paid')
