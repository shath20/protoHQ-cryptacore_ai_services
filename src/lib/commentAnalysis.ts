...
export function analyzeComment(comment: Comment, settings?: RiskSettings): number {
  let score = 0;
...
export function generateIndicators(comment: Comment, score: number, settings?: RiskSettings): string[] {
  const indicators = [];
  const content = comment.content.toLowerCase();
  const originalContent = comment.content;
